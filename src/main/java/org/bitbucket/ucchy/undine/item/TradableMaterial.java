...
//    STRUCTURE_BLOCK(255),
    ...
    private TradableMaterial(final int id) {
        this.id = id;
    }

    /**
     * 文字列からTradableMaterialを返す
     * @param name 文字列
    ...
    /**
     * TradableMaterialに変換して返す
     * @param material
     * @return TradableMaterial
     */
    public static TradableMaterial convertFromMaterial(Material material) {
        return TradableMaterial.getMaterial(material.toString());
    }
    ...
    /**
     * 指定されたアイテムIDに一致するTradableMaterialを返す
     * @param id アイテムID
     * @return TradableMaterial
     */
    public static TradableMaterial getFromId(int id) {
        for ( TradableMaterial m : values() ) {
            if ( m.getId() == id ) {
                return m;
            }
